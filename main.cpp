...
int main() {
    ...
    int n_threads = 2; // Количество используемых потоков
    unsigned int max_hardware_threads = std::thread::hardware_concurrency();
    if (max_hardware_threads == 0) max_hardware_threads = 1; //На случай проблем с доступом
    ...
    while (true) {
        ...
        std::cout << "4. Изменить количество ядер\n";
        ...
        if (inputNumber == "1") {
            ...
            std::cout << "Максимальное количество доступных ядер: " << max_hardware_threads << std::endl;
            std::cout << "Текущее количество ядер: " << n_threads << std::endl;
            std::cout << "Введите новое количество ядер (или 0 для возврата): ";
            int new_n_threads;
            std::cin >> new_n_threads;

            if (new_n_threads == 0) {
                std::cout << "Возврат в меню.\n";
            } else if (new_n_threads < 0) {
                std::cout << "Количество ядер не может быть отрицательным.\n";
            } else if (new_n_threads > static_cast<int>(max_hardware_threads)) {
                std::cout << "Количество ядер превышает максимальное доступное (" << max_hardware_threads << "). Установлено: " << max_hardware_threads << std::endl;
                n_threads = max_hardware_threads;
            } else {
                n_threads = new_n_threads;
                std::cout << "Количество ядер установлено: " << n_threads << std::endl;
            }
